...
# JUMBOTRON startup script
#
# Use this file for SYSVINIT-style startup.
# Copy it to /etc/init.d/jumbotron
#     To start  : /etc/init.d/jumbotron start
#     To stop   : /etc/init.d/jumbotron start
#     For status: /etc/init.d/jumbotron status
# To lauch at boot and stopp at shutdown:
#     Install   : update-rc.d jumbotron defaults 
#     Uninstall : update-rc.d -f jumbotron remove
...
# ------------------------------------------------------------------------------
#  Based on : Debian /etc/init.d/skeleton
...
# Description:       junkyard jumbotrom server
...
DAEMON_DIR=/home/jumbotron/jumbotron
NAME=node
DAEMON=$DAEMON_DIR/local/bin/$NAME
DAEMON_ARGS=jjserver.js

DESC="junkyard-jumbotron"                   # whatever fancy description you like
...
LOCAL_VAR_RUN=/var/run			    # Need to run as root for socket.io
...
do_start()
{
    ...
    start-stop-daemon --start --quiet --pidfile $PIDFILE --chuid $NODEUSER --background --chdir $DAEMON_DIR --exec $DAEMON --test > /dev/null \
	|| { [ "$VERBOSE" != no ] && log_daemon_msg  "  --->  Daemon already running $DESC" "$INIT_SCRIPT_NAME_NOEXT"; return 1; }
    start-stop-daemon --start --quiet --chuid $NODEUSER --make-pidfile --pidfile $PIDFILE --background --chdir $DAEMON_DIR --exec $DAEMON -- \
	$DAEMON_ARGS \
...
do_stop()
{
	...
	# sleep for some time.
	start-stop-daemon --stop --quiet --oknodo --retry=0/3/KILL/5 --pidfile $PIDFILE  --chuid $NODEUSER --chdir $DAEMON_DIR --exec $DAEMON -- $DAEMON_ARGS
	[ "$?" = 2 ] && return 2
...
running() {
  RUNSTAT=$(start-stop-daemon --start --quiet --pidfile $PIDFILE --chuid $NODEUSER --background --chdir $DAEMON_DIR --exec $DAEMON --test > /dev/null)
  if [ "$?" = 1 ]; then
    echo y
  fi
